...
private fun PreviewBookList() {
    AppTheme {
        ...
        BookListScreen(modifier = Modifier.fillMaxSize(), books = testBookData, {}) {
        }
...
fun MyBookScreen(modifier: Modifier = Modifier) {
    ...
    val viewModel: MyBookViewModel =
        viewModel(factory = MyBookViewModelFactory(LocalContext.current))

    val wordBookList = viewModel.wordBookList.collectAsState().value

    CommonLoadingLayout(data = wordBookList) { books ->
...
private fun BookListScreen(
    ...
    onRemove: (WordBookUI) -> Unit,
    onClick: (WordBookUI) -> Unit
) {
...
@Composable
private fun WordBookItem(modifier: Modifier = Modifier, book: WordBookUI, onRemove: () -> Unit) {
    ...
    Row(modifier = modifier) {
        ...
        Column(modifier = Modifier.fillMaxHeight()) {
            ...
            Row(verticalAlignment = Alignment.Bottom) {
                ...
                Button(
                    onClick = { onRemove() }, contentPadding = PaddingValues(0.dp),
